...
fun longest_string1 los = 
   List.foldl (fn (s1,s2) => if String.size s1 > String.size s2 then s1 else s2) "" los
...
fun longest_string_helper f los=
  List.foldl (fn (s1,s2) => if f(String.size s1, String.size s2) then s1 else s2) "" los
...
* (notice the 2 arguments are curried). Tihe first argument should be applied to elements of the second argument. 
...
let fun helper (SOME x) (SOME l)= SOME (l @ x)
      | helper _ _ = NONE
in
   foldl (fn (x, acc) => helper (f x) acc) (SOME []) lst
end
