	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(x, y, e) => case e of 
			     ...
			     | Line(m, b) => Line(m, b + y - m * x)
                             ...
                             | LineSegment(x1, y1, x2, y2) => LineSegment(x1 + x, y1 + y, x2 + x, y2 + y)				| _ => eval_prog(Shift(x, y, eval_prog(e, env)), env)
  ...
  case e of
     ...
     | LineSegment (x1,y1,x2,y2) => if real_close_point (x1, y1) (x2, y2) 
                                    then Point(x1,y1)
                                      ...
                                      if x1 - x2 > epsilon orelse real_close(x1,x2) andalso y1-y2 > epsilon
